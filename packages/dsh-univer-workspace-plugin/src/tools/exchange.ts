...
export function registerExchangeTools(ctx: Context): () => void {
  const disposeImport = registerUniverTool(
    ctx,
    defineTool({
      name: "univer_import",
      description:
      ...
      parameters: {
        source: {
          type: "string",
          required: true,
          description: "Path to the Office source file relative to the session workspace.",
        },
        file: {
          type: "string",
          description:
            "Optional Office-compatible logical target path; remote Units are identified by the returned unitId/resourceId.",
        },
        worktreeId: { type: "string", required: true, description: "Writable draft Worktree id." },
        name: {
          type: "string",
          required: true,
          description: "Non-empty name for the imported Unit.",
        },
        spaceId: {
          type: "string",
          description:
            "Optional target Space; omit to use the linked Space, or provide a Space accessible to the authenticated User.",
        },
        parentNodeId: { type: "string", description: "Optional target folder Node id." },
        unitType: {
          ...unitTypeEnum,
          description: "Optional explicit type; otherwise infer it from the source extension.",
        },
        type: {
          type: "string",
          enum: ["auto", "sheet", "doc", "slide", "base"],
          description: "Deprecated alias for unitType.",
        },
        idempotencyKey: {
          type: "string",
          description: "Optional stable key for retry-safe Worktree Unit creation.",
        },
      },
      output: {
        schema: { type: "json" },
      ...
      async execute(args: ImportArguments, exec: ToolRunContext): Promise<JsonValue> {
        ...
        const worktreeId = nonEmpty(
          args.worktreeId,
          "univer_import requires a non-empty worktreeId",
        );
        ...
        if (args.idempotencyKey !== undefined && idempotencyKey === "") {
          throw new UniverError(
            "univer_import idempotencyKey must be non-empty.",
            "INVALID_REQUEST",
          );
        }
  ...
  const disposeExport = registerUniverTool(
    ctx,
    defineTool({
      name: "univer_export",
      description:
      ...
      parameters: {
        file: {
          type: "string",
          description:
            "Optional Office-compatible logical source path; not required for a remote Unit.",
        },
        output: {
          type: "string",
          description:
            "Destination path relative to the session workspace (preferred Office-compatible spelling).",
        },
        ...
        worktreeId: {
          type: "string",
          description: "Optional Worktree scope; omit to export the linked Space's trunk Unit.",
        },
        type: {
          ...unitTypeEnum,
          description: "Optional type assertion; the server-resolved Unit type is authoritative.",
        },
        format: {
          type: "string",
          enum: ["xlsx", "csv", "tsv", "docx", "pptx"],
          description: "Optional format assertion; normally inferred from output extension.",
        },
        revision: {
          type: "integer",
          description: "Optional expected revision; reject export if the Unit has advanced.",
        },
      },
      output: {
        schema: { type: "json" },
      ...
      async execute(args: ExportArguments, exec: ToolRunContext): Promise<JsonValue> {
        ...
        const outputArg = nonEmpty(
          args.output ?? (args.output === undefined ? args.file : undefined),
          "univer_export requires output",
        );
        const worktreeId =
          args.worktreeId === undefined
            ? undefined
            : nonEmpty(args.worktreeId, "univer_export worktreeId must be non-empty");
        const target = await resolveUnitTarget(ctx, resolved, unitId, worktreeId);
        if (args.type !== undefined && args.type !== target.unitType) {
          throw new UniverError(
            `The Unit type is ${target.unitType}, not ${args.type}.`,
            "UNIT_TYPE_MISMATCH",
          );
        }
...
function explicitImportType(
  ...
  if (unitType !== undefined && legacyType !== undefined && unitType !== legacyType) {
    throw new UniverError(
      `unitType ${unitType} conflicts with type ${legacyType}.`,
      "INVALID_REQUEST",
    );
  }
...
async function resolveUnitTarget(
  ...
  if (worktreeId === undefined) {
    ...
    if (document.unitType === "board") {
      throw new UniverError(
        "Board Units do not have an Office exchange format.",
        "EXCHANGE_FORMAT_UNSUPPORTED",
      );
    }
  ...
  if (unit === undefined)
    throw new UniverError("The Unit is not part of the requested Worktree.", "UNIT_NOT_FOUND");
