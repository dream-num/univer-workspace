...
export async function resolveExecutionCode(
  ...
  try {
    ...
    throw new UniverError("Cannot read the requested codeFile.", "CODE_FILE_READ_FAILED", {
      cause: error,
    });
  }
}
...
async function executeUnit(
  ...
  const runtimeScope: WorkspaceRuntimeScope =
    args.worktreeId === undefined
      ? { kind: "trunk" }
      : { kind: "worktree", worktreeId: args.worktreeId };
  if (mode === "read") {
...
export async function assertUnitInScope(
  ...
  if (unitId.trim() === "")
    throw new UniverError("Univer tools require a non-empty unitId.", "INVALID_REQUEST");
  ...
  if (worktreeId === undefined) {
    ...
    if (document.unitType !== unitType) {
      throw new UniverError(
        `The Unit type is ${document.unitType}, not ${unitType}.`,
        "UNIT_TYPE_MISMATCH",
      );
    }
    return;
  ...
  if (worktreeId.trim() === "")
    throw new UniverError("Univer tools require a non-empty worktreeId.", "INVALID_REQUEST");
  ...
  if (unit.unitType !== unitType) {
    throw new UniverError(
      `The Unit type is ${unit.unitType}, not ${unitType}.`,
      "UNIT_TYPE_MISMATCH",
    );
  }
...
export function registerEditTool(ctx: Context): () => void {
  const disposeTool = registerUniverTool(
    ctx,
    defineTool({
      name: "univer_edit",
      description:
  ...
  const disposeInspect = registerUniverTool(
    ctx,
    defineTool({
      name: "univer_inspect",
      description:
      ...
      parameters: {
        ...
        range: {
          type: "string" as const,
          description:
            "Optional case-sensitive Sheet range, e.g. 'Sheet 1'!A1:D20. Quote worksheet names containing spaces; omit it first to discover names.",
        },
      },
      output: {
        schema: {
      ...
      presentCall: (args) => ({
        card: "generic",
        title:
          args.range === undefined ? "Inspect Univer Unit" : `Inspect Univer range ${args.range}`,
        kind: "read",
      }),
    }),
  );

  const disposeExecute = registerUniverTool(
    ctx,
    defineTool({
      name: "univer_execute",
      description:
      ...
      presentCall: () => ({
        card: "generic",
        title: "Execute Univer Facade code",
        kind: "execute",
      }),
    }),
  );
