  ...
  const disposers = [
    registerUniverTool(
      ...
      defineTool({
        name: "univer_open",
        description:
          "Open a Univer document (by resourceId) to learn its unitId, unitType, and whether it is editable.",
        parameters: {
        ...
        async execute(args, exec) {
          ...
          const document = await ctx
            .get("univerWorkspace")!
            .openDocument(resolved.userId, args.resourceId);
          return document;
        },
    ...
    registerUniverTool(
      ...
      defineTool({
        ...
        parameters: {
          ...
          unitId: {
            type: "string",
            description: "Optional Unit filter inside the selected Worktree.",
          },
        },
        output: {
          schema: { type: "json" },
        ...
        async execute(args, exec) {
          ...
          if (resourceId === undefined && worktreeId === undefined && unitId === undefined) {
            throw new UniverError(
              "univer_status requires resourceId, unitId, or worktreeId.",
              "INVALID_REQUEST",
            );
          }
          ...
          if (worktreeId === undefined && resourceId === undefined && unitId !== undefined) {
            const document = await ctx
              .get("univerWorkspace")!
              .resolveUnitResource(resolved.userId, unitId);
            const fileState = await ctx
              .get("univerWorkspace")!
              .getFileState(resolved.userId, document.resourceId);
            return {
              ...document,
          ...
          if (worktreeId !== undefined) {
            ...
            const fileState = await ctx
              .get("univerWorkspace")!
              .getWorktreeFileState(resolved.userId, worktreeId);
            const worktree = fileState.worktrees.find((entry) => entry.worktreeId === worktreeId);
            if (worktree === undefined) {
              throw new UniverError(
                "univer_status Worktree state is unavailable.",
                "WORKTREE_NOT_FOUND",
              );
            }
            const selected =
            ...
            if (unitId !== undefined && selected === undefined) {
              throw new UniverError(
                "The requested Unit is not part of the selected Worktree.",
                "UNIT_NOT_FOUND",
              );
            }
            if (resourceId !== undefined && selected === undefined) {
              throw new UniverError(
                "The requested Resource is not part of the selected Worktree.",
                "WORKSPACE_NOT_FOUND",
              );
            }
            if (
              ...
              throw new UniverError(
                "The requested Resource does not belong to the selected Unit.",
            ...
            if (selected !== undefined && selected.source === "trunk") {
              document = await ctx
                .get("univerWorkspace")!
                .openDocument(resolved.userId, selected.resourceId);
            ...
            return {
              ...
              ...(selected === undefined
                ...
                : {
                    ...
                    name: selected.name,
                    editorMode: (selected.source === "worktree" && worktree.status === "draft"
                      ? "edit"
                      : "readOnly") as "edit" | "readOnly",
                  }),
              worktreeId,
              document: document as unknown as JsonValue,
          ...
          const document = await ctx
            .get("univerWorkspace")!
            .openDocument(resolved.userId, trunkResourceId);
          if (unitId !== undefined && unitId !== document.unitId) {
            throw new UniverError(
              "The requested Unit does not belong to the selected Resource.",
              "UNIT_NOT_FOUND",
            );
          }
          const fileState = await ctx
            .get("univerWorkspace")!
            .getFileState(resolved.userId, trunkResourceId);
          return {
            ...document,
            document: document as unknown as JsonValue,
            unit: document as unknown as JsonValue,
