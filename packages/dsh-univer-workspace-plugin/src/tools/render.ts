...
export function registerRenderTools(
  ...
  const disposeLint = registerUniverTool(ctx, defineTool({
    name: "univer_lint",
    description:
    ...
    parameters: {
      ...
      unitType: { ...unitTypeEnum, required: true, description: "Must be slide for layout lint." },
      worktreeId: { type: "string", description: "Optional draft Worktree scope; omit for trunk." },
      pages: {
        type: "array",
    ...
    async execute(args: LintArguments, exec: ToolRunContext): Promise<JsonValue> {
      ...
      const result = await lintUnitLayout(source, args.pages as readonly SlideLayoutLintPageSelector[] | undefined, {
        env: process.env,
        signal: exec.signal,
      });
      return {
  ...
  const disposeScreenshot = options.includeScreenshot === false
    ? () => undefined
    : registerScreenshotTool(ctx);

  const disposeCompile = registerUniverTool(ctx, defineTool({
    ...
    parameters: {
      source: { type: "string", required: true, description: "SVG path inside the session workspace." },
...
export function registerScreenshotTool(ctx: Context): () => void {
  return registerUniverTool(ctx, defineTool({
    ...
    parameters: {
      ...
      worktreeId: { type: "string", description: "Optional draft Worktree scope; omit for trunk." },
      output: { type: "string", required: true, description: "Session-relative output directory for PNG files." },
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown): ContentBlock[] => {
        ...
        return [
          ...
          ...images.flatMap((image) => image.image === undefined ? [] : [{
            type: "image" as const,
    ...
    async execute(args: ScreenshotArguments, exec: ToolRunContext) {
      ...
      const scope = args.worktreeId === undefined
        ? { kind: "trunk" as const }
      ...
      try {
        refs = await attachments.saveImages(decoded.map(({ image, bytes }) => ({
...
function assertAttachmentPolicy(attachments: AttachmentStoreLike): void {
  ...
  if (!Number.isSafeInteger(maxMessageImageBytes) || maxMessageImageBytes < 1
    || !Number.isSafeInteger(maxImageBytes) || maxImageBytes < 1) {
    throw new UniverError(
...
function validateAttachmentLimits(
  ...
  images: readonly { readonly image: { readonly name: string; readonly width: number; readonly height: number; readonly bytes: number }; readonly bytes: Uint8Array }[],
): void {
...
async function assertImageCapableRoute(ctx: Context, exec: ToolRunContext, identity: string): Promise<void> {
  const agent = exec.agent as unknown as {
    readonly session?: { readonly requestHeader?: () => { readonly config?: { readonly provider?: string; readonly model?: string } } | undefined };
...
async function loadUnitSource(
  ...
  exec: ToolRunContext,
  args: LintArguments,
): Promise<{ readonly unitId: string; readonly unitType: UnitType; readonly unitData: Awaited<ReturnType<NonNullable<Context["univerWorkspace"]>["exportUnitData"]>> }> {
  const resolved = await resolveToolScope(ctx, exec);
  await assertUnitInScope(ctx, resolved, args.unitId, args.unitType, args.worktreeId);
  const scope = args.worktreeId === undefined
    ? { kind: "trunk" as const }
...
function screenshotTarget(args: ScreenshotArguments): UnitScreenshotInput["target"] | undefined {
  ...
  const usesPaged = args.pages !== undefined || args.contactSheet === true || args.tileColumns !== undefined || args.tileRows !== undefined;
  ...
  if (args.unitType === "doc" && usesPaged) {
    ...
    if (pages !== undefined && pages.some((page) => typeof page !== "number")) {
      throw new UniverError("Doc screenshots accept numeric pages only.", "SCREENSHOT_INPUT_INVALID");
    }
    if (args.contactSheet === true || args.tileColumns !== undefined || args.tileRows !== undefined) {
      throw new UniverError("Doc screenshots do not support contact sheets.", "SCREENSHOT_INPUT_INVALID");
    }
    return { kind: "doc-pages", ...(pages === undefined ? {} : { pages: pages as readonly number[] }), ...(args.scale === undefined ? {} : { scale: args.scale }) } as UnitScreenshotInput["target"];
  }
  if (args.unitType === "slide" && usesPaged) {
    ...
    return {
      ...
      ...(args.contactSheet === true
        ? { contactSheet: hasColumns && hasRows ? { tile: { columns: args.tileColumns!, rows: args.tileRows! } } : {} }
        : {}),
      ...(args.scale === undefined ? {} : { scale: args.scale }),
    } as UnitScreenshotInput["target"];
  }
  ...
  if (args.unitType === "sheet") return { kind: "sheet-viewport", scale: args.scale } as UnitScreenshotInput["target"];
  if (args.unitType === "doc") return { kind: "doc-pages", scale: args.scale } as UnitScreenshotInput["target"];
  if (args.unitType === "slide") return { kind: "slide-pages", scale: args.scale } as UnitScreenshotInput["target"];
  if (args.unitType === "base") return { kind: "base-view", scale: args.scale } as UnitScreenshotInput["target"];
