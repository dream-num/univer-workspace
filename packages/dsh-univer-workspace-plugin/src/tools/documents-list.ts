...
function listOutput(documents: readonly WorkspaceDocument[], spaceId: string): {
  spaceId: string;
  documents: ListedDocument[];
  folders: ListedFolder[];
} {
  const folders = documents.filter(document => document.resourceKind === null).map(folder => ({
    ...
    ...(folder.nodeCapabilities === undefined ? {} : { nodeCapabilities: folder.nodeCapabilities }),
  }));
  const listed = documents.filter(document => document.resourceKind !== null && document.resourceId !== null).map(document => ({
    ...
    ...(document.nodeCapabilities === undefined ? {} : { nodeCapabilities: document.nodeCapabilities }),
    ...(document.resourceCapabilities === undefined ? {} : { resourceCapabilities: document.resourceCapabilities }),
  }));
  return { spaceId, documents: listed, folders };
}

const listSchema = {
  spaceId: { type: "string" as const, description: "Optional. Omit to list the Space linked to this session; an explicit value may select any Space accessible to the authenticated User." },
  ...
  properties: {
    ...
    documents: {
      ...
      items: {
        ...
        additionalProperties: false,
        properties: {
          nodeId: { type: "string" as const, required: true }, name: { type: "string" as const, required: true },
          parentNodeId: { oneOf: [{ type: "string" as const }, { type: "null" as const }] as const, required: true }, hasChildren: { type: "boolean" as const, required: true },
          updatedAt: { oneOf: [{ type: "string" as const }, { type: "null" as const }] as const, required: true }, resourceId: { type: "string" as const, required: true },
          resourceKind: { type: "string" as const, enum: ["univer", "blob"] as const, required: true },
          unitType: { type: "string" as const }, mediaType: { type: "string" as const },
          byteSize: { type: "number" as const }, availability: { type: "string" as const },
          accessRole: { type: "string" as const, required: true }, nodeCapabilities: { type: "json" as const },
    ...
    folders: {
      ...
      items: {
        ...
        properties: {
          nodeId: { type: "string" as const, required: true }, name: { type: "string" as const, required: true },
          parentNodeId: { oneOf: [{ type: "string" as const }, { type: "null" as const }] as const, required: true }, hasChildren: { type: "boolean" as const, required: true },
          updatedAt: { oneOf: [{ type: "string" as const }, { type: "null" as const }] as const, required: true }, accessRole: { type: "string" as const, required: true },
...
const executeList = async (ctx: Context, args: ListArguments, exec: ToolRunContext) => {
  ...
  const documents = await ctx.get("univerWorkspace")!.listDocuments(resolved.userId, spaceId, listOptions(args));
  return listOutput(documents, spaceId);
};

const renderList = (_args: ListArguments, value: unknown) => {
  const record = (value ?? {}) as { documents?: { name: string; unitType?: string; resourceKind?: string; resourceId: string }[]; folders?: { name: string }[] };
  const lines = [
    ...(record.folders ?? []).map(folder => `📁 ${folder.name}`),
    ...(record.documents ?? []).map(document => `${document.name} (${document.resourceKind ?? "resource"}${document.unitType === undefined ? "" : `/${document.unitType}`}, ${document.resourceId})`),
  ];
...
export function registerDocumentListTools(ctx: Context): () => void {
  const disposers = [
    ...
    registerUniverTool(ctx, defineTool({
      name: "univer_list",
      description: "Alias of univer_documents. Lists the linked Space by default and accepts any other Space accessible to the authenticated User. Supports the same filters as univer_documents.",
      parameters: listSchema,
      output: { schema: listOutputSchema, render: renderList },
      execute: (args, exec) => executeList(ctx, args, exec),
      presentCall: () => ({ card: "generic", title: "List Workspace documents", kind: "read" }),
