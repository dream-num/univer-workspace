...
export interface WorktreeSummary {
  ...
  readonly creator: WorktreeCreator | null;
...
export function narrowWorktreeSummary(raw: unknown): WorktreeSummary {
  ...
  const worktree = (record.worktree !== null && typeof record.worktree === "object"
    ? record.worktree
  ...
  const summary = worktree.summary === null || typeof worktree.summary === "string" ? worktree.summary ?? null : null;
  const kind = worktree.kind === "team" ? "team" : "user";
  const teamSpaceRaw = worktree.teamSpace;
  const teamSpace = teamSpaceRaw !== null && typeof teamSpaceRaw === "object"
    ...
    : null;
  const visibility = worktree.visibility === "space" ? "space" : "private";
  const creatorRaw = worktree.creator;
  const creator = creatorRaw !== null && typeof creatorRaw === "object"
    ...
    && ((creatorRaw as Record<string, unknown>).avatarUrl === null || typeof (creatorRaw as Record<string, unknown>).avatarUrl === "string")
    ? {
  ...
  const capability = (key: keyof WorktreeCapabilities): boolean =>
    capabilitiesRaw !== null && typeof capabilitiesRaw === "object"
      && (capabilitiesRaw as Record<string, unknown>)[key] === true;
  const unitCount = typeof worktree.unitCount === "number" && Number.isSafeInteger(worktree.unitCount) && worktree.unitCount >= 0
    ? worktree.unitCount
    : 0;
  const timestamp = (key: "processedAt" | "createdAt" | "updatedAt"): string | null =>
    worktree[key] === null || typeof worktree[key] === "string" ? worktree[key] as string | null ?? null : null;
  return {
    id,
    name,
    summary,
...
export interface WorktreeUnitDescriptor {
  ...
  readonly activationState: "notApplicable" | "waitingForMerge" | "pending" | "completed" | "failed" | "discarded";
}
...
const WORKTREE_ACTIVATION_STATES = ["notApplicable", "waitingForMerge", "pending", "completed", "failed", "discarded"] as const;
...
export function narrowWorktreeUnit(raw: unknown): WorktreeUnitDescriptor {
  ...
  const source = record.source === "trunk" || record.source === "worktree" ? record.source : undefined;
  ...
  if (targetRaw === null) {
    ...
    const parentNodeId = targetRecord.parentNodeId === null
      ? null
      : stringField(targetRecord.parentNodeId);
    if (spaceId === undefined || parentNodeId === undefined) {
      throw new WorkspaceApiError("workspace worktree unit returned a malformed target", 502, "MALFORMED_WORKTREE_UNIT");
    }
    target = { spaceId, parentNodeId };
  } else {
    target = undefined;
  }
  const draftHeadRevision = typeof record.draftHeadRevision === "number" && Number.isInteger(record.draftHeadRevision) && record.draftHeadRevision >= 0
  ...
  if (unitId === undefined || resourceId === undefined || nodeId === undefined || source === undefined || name === undefined || unitType === undefined || target === undefined || draftHeadRevision === undefined || change === undefined || mergeResult === undefined || activationState === undefined) {
    throw new WorkspaceApiError("workspace worktree unit returned a malformed descriptor", 502, "MALFORMED_WORKTREE_UNIT");
  }
  if ((source === "trunk" && target !== null) || (source === "worktree" && target === null)) {
    throw new WorkspaceApiError("workspace worktree unit source and target do not match", 502, "MALFORMED_WORKTREE_UNIT");
  }
  return { unitId, resourceId, nodeId, source, name, unitType, target, draftHeadRevision, change, mergeResult, activationState };
}
...
function narrowOperation(raw: unknown): OperationView {
  const record = (raw ?? {}) as Record<string, unknown>;
  const operation = record.operation !== null && typeof record.operation === "object"
    ? record.operation as Record<string, unknown>
    : record;
  const id = stringField(operation.id);
  const state = operation.state === "pending" || operation.state === "completed" || operation.state === "failed"
    ? operation.state
    : undefined;
  const result = operation.result === null || operation.result === undefined
    ? null
    : operation.result !== null && typeof operation.result === "object" && !Array.isArray(operation.result)
      ? operation.result as Readonly<Record<string, unknown>>
      : undefined;
  const errorRaw = operation.error;
  const error = errorRaw === null || errorRaw === undefined
    ? null
  ...
  if (id === undefined || state === undefined || result === undefined || error === undefined) {
    throw new WorkspaceApiError("workspace operation returned a malformed descriptor", 502, "MALFORMED_OPERATION");
  }
...
async function awaitOperation(client: WorkspaceHttpClient, operationId: string): Promise<OperationView> {
...
export async function createWorktreeLocalUnit(
  ...
  if (response.status === 202) {
    ...
    const completed = operation.state === "pending" ? await awaitOperation(client, operation.id) : operation;
    if (completed.state !== "completed") {
      throw new WorkspaceApiError("workspace worktree unit create did not complete", 502, "WORKTREE_UNIT_CREATE_FAILED");
    }
    ...
    const unitsRaw = ((detail as Record<string, unknown>).worktree as Record<string, unknown> | undefined)?.units;
    const units = Array.isArray(unitsRaw) ? unitsRaw : [];
    const resultUnitId = typeof completed.result?.unitId === "string" ? completed.result.unitId : undefined;
    ...
    if (rawUnit === undefined) {
      throw new WorkspaceApiError("workspace operation completed without a Worktree Unit", 502, "MALFORMED_WORKTREE_UNIT");
    }
...
export async function reopenWorktree(client: WorkspaceHttpClient, worktreeId: string): Promise<WorktreeSummary> {
  const raw = await readJson(
    await client.request(`/api/worktrees/${encodeURIComponent(worktreeId)}/reopen`, jsonWithIdempotency({})),
    "worktree reopen",
  );
  return narrowWorktreeSummary(raw);
}

/** One changed unit inside a worktree, as the review surfaces render it. */
export interface WorktreeUnitView {
  readonly unitId: string;
  readonly resourceId: string;
  readonly name: string;
  ...
  readonly source: "trunk" | "worktree";
  readonly target: { readonly spaceId: string; readonly parentNodeId: string | null } | null;
  readonly kind: "modified" | "added" | "deleted" | "unchanged";
  readonly draftHeadRevision: number;
}

/** A worktree with its changed units. */
export interface WorktreeStateView {
  readonly worktreeId: string;
  readonly name: string;
  readonly status: WorktreeSummary["state"];
  readonly summary: string | null;
  readonly kind: WorktreeSummary["kind"];
  readonly teamSpace: WorktreeSummary["teamSpace"];
  readonly visibility: WorktreeSummary["visibility"];
  readonly creator: WorktreeSummary["creator"];
  readonly unitCount: number;
  readonly processedAt: string | null;
  readonly createdAt: string | null;
  readonly updatedAt: string | null;
...
export function narrowWorktreeDetail(raw: unknown): WorktreeStateView {
  ...
  const rawUnits = Array.isArray(worktree.units) ? worktree.units : [];
  const units: WorktreeUnitView[] = [];
  for (const entry of rawUnits) {
    if (entry === null || typeof entry !== "object") {
      throw new WorkspaceApiError("workspace worktree detail contains a malformed Unit", 502, "MALFORMED_WORKTREE_UNIT");
    }
...
/** List the current User's Worktrees in one product lifecycle scope. */
async function listWorktrees(
  client: WorkspaceHttpClient,
  scope: "active" | "processed",
): Promise<readonly WorktreeStateView[]> {
  const states: WorktreeStateView[] = [];
  ...
  for (;;) {
    ...
    if (!Array.isArray(record.items)) {
      throw new WorkspaceApiError("workspace worktree list returned no items array", 502, "MALFORMED_WORKTREES");
    }
    for (const entry of record.items) {
      const id = stringField((entry as Record<string, unknown> | null)?.id);
      if (id === undefined) continue;
      const detail = await readJson(
        await client.request(`/api/worktrees/${encodeURIComponent(id)}`),
        "worktree detail",
      );
      states.push(narrowWorktreeDetail(detail));
    }
    if (record.nextCursor === null || record.nextCursor === undefined) break;
    if (typeof record.nextCursor !== "string" || record.nextCursor === cursor) {
      throw new WorkspaceApiError("workspace worktree list returned an invalid nextCursor", 502, "MALFORMED_WORKTREES");
    }
...
export async function listActiveWorktrees(client: WorkspaceHttpClient): Promise<readonly WorktreeStateView[]> {
  return await listWorktrees(client, "active");
}
 ...
 */
export async function listReviewWorktrees(client: WorkspaceHttpClient): Promise<readonly WorktreeStateView[]> {
  const [active, processed] = await Promise.all([
    listWorktrees(client, "active"),
    listWorktrees(client, "processed"),
  ]);
  const byId = new Map<string, WorktreeStateView>();
  for (const worktree of [...active, ...processed]) byId.set(worktree.worktreeId, worktree);
  return [...byId.values()];
}
