...
describe("Workspace Worktree contract", () => {
  ...
  it.each(["draft", "ready", "merged", "discarded"])(
    "keeps deletion-only %s Worktrees reviewable without opening removed resources",
    async (state) => {
      const calls: string[] = [];
      const client = {
      ...
      const result = await getWorktreeFileState(client, "wt-1");
      expect(result.worktrees[0]?.units[0]?.kind).toBe("deleted");
      expect(result.viewerTarget).toBeNull();
