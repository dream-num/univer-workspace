...
import {
  createDocument, createWorktreeLocalUnit, getWorktreeDetail, listSpaceDocuments, narrowNodePage,
  narrowSpaces, newIdempotencyKey, narrowNodes, narrowUnitResource,
...
describe("dsh-univer-workspace-plugin", () => {
  ...
  it("renders the session-linked Space so the agent does not guess across Spaces", () => {
    const rendered = renderSpaces([
      { spaceId: "sp-personal", type: "personal", name: "Personal", accessRole: "owner", dshWorkspaceId: "ws-1", linked: false },
      { spaceId: "sp-team", type: "team", name: "Team", accessRole: "editor", dshWorkspaceId: "ws-2", linked: true },
    ]);
    expect(rendered).toEqual([{ type: "text", text: "Personal (personal, sp-personal)\nTeam (team, sp-team, linked to this session)" }]);
  });
  ...
  it("walks every paged root and nested Node page", async () => {
      ...
      async request(path) {
        ...
        if (path === "/api/spaces/sp-1/nodes?limit=100") {
          return jsonResponse(200, {
            nodes: [
              { id: "folder-a", spaceId: "sp-1", parentNodeId: null, name: "Folder A", resource: null, hasChildren: true },
              { id: "root-doc", spaceId: "sp-1", parentNodeId: null, name: "Root", resource: { id: "res-root", kind: "univer", unitType: "sheet" }, hasChildren: false },
            ],
        ...
        if (path === "/api/spaces/sp-1/nodes?limit=100&cursor=root-next") {
          return jsonResponse(200, {
            nodes: [{ id: "folder-b", spaceId: "sp-1", parentNodeId: null, name: "Folder B", resource: null, hasChildren: true }],
            nextCursor: null,
          });
        }
        if (path === "/api/nodes/folder-a/children?limit=100") {
          return jsonResponse(200, {
            nodes: [{ id: "nested-doc", spaceId: "sp-1", parentNodeId: "folder-a", name: "Nested", resource: { id: "res-nested", kind: "univer", unitType: "doc" }, hasChildren: false }],
            nextCursor: "child-next",
          });
        }
        if (path === "/api/nodes/folder-a/children?limit=100&cursor=child-next") {
          return jsonResponse(200, {
            nodes: [{ id: "nested-folder", spaceId: "sp-1", parentNodeId: "folder-a", name: "Nested Folder", resource: null, hasChildren: true }],
            nextCursor: null,
          });
        }
        if (path === "/api/nodes/folder-b/children?limit=100") {
          return jsonResponse(200, {
            nodes: [{ id: "deep-doc", spaceId: "sp-1", parentNodeId: "folder-b", name: "Deep", resource: { id: "res-deep", kind: "univer", unitType: "slide" }, hasChildren: false }],
            nextCursor: null,
          });
        }
        if (path === "/api/nodes/nested-folder/children?limit=100") {
          return jsonResponse(200, {
            nodes: [{ id: "deep-nested-doc", spaceId: "sp-1", parentNodeId: "nested-folder", name: "Deep nested", resource: { id: "res-deep-nested", kind: "univer", unitType: "base" }, hasChildren: false }],
            nextCursor: null,
          });
        }
    ...
    expect(documents.map(document => document.nodeId)).toEqual([
      "folder-a", "root-doc", "folder-b", "nested-doc", "nested-folder", "deep-doc", "deep-nested-doc",
    ]);
    expect(documents.find(document => document.nodeId === "nested-doc")).toMatchObject({
      parentNodeId: "folder-a", resourceId: "res-nested", unitType: "doc",
    });
  ...
  it("supports hierarchy, query, resource-kind, and unit-type filters without dropping blobs", async () => {
    const client: WorkspaceHttpClient = {
      ...
      async request(path) {
        if (path === "/api/spaces/sp-1/nodes?limit=100") {
          return jsonResponse(200, {
            nodes: [
              { id: "folder", spaceId: "sp-1", parentNodeId: null, name: "Reports", resource: null, hasChildren: true },
              { id: "blob", spaceId: "sp-1", parentNodeId: null, name: "Report.csv", resource: { id: "blob-1", kind: "blob", mediaType: "text/csv", byteSize: 12, availability: "ready", capabilities: {} }, hasChildren: false },
            ],
            nextCursor: null,
          });
        }
        if (path === "/api/nodes/folder/children?limit=100") {
          return jsonResponse(200, {
            nodes: [{ id: "sheet", spaceId: "sp-1", parentNodeId: "folder", name: "Monthly", resource: { id: "res-1", kind: "univer", unitType: "sheet", capabilities: {} }, hasChildren: false }],
            nextCursor: null,
          });
        }
        throw new Error(`unexpected request ${path}`);
    ...
    expect(all.find(document => document.resourceKind === "blob")).toMatchObject({ resourceId: "blob-1", mediaType: "text/csv", byteSize: 12 });
    const nestedSheets = await listSpaceDocuments(client, "sp-1", { parentNodeId: "folder", recursive: true, query: "month", unitType: "sheet" });
  ...
  it("rejects a malformed node cursor instead of looping", () => {
    expect(() => narrowNodePage({ nodes: [], nextCursor: 42 }, "sp-1")).toThrow(/invalid nextCursor/);
  });
  ...
  it("normalizes the unit-resource summary contract without requiring open-only fields", () => {
    const resolved = narrowUnitResource({
      ...
      node: {
        ...
        spaceId: "sp-1",
        name: "Resolved Sheet",
        resource: { id: "res-4", kind: "univer", unitType: "sheet", capabilities: { editContent: true } },
        accessRole: "editor",
      },
    }, "unit-4");
  ...
  it("rejects a unit-resource response whose Node and Resource identities disagree", () => {
    ...
    }, "unit-4")).toThrow(/malformed descriptor/);
  });
    ...
    const client: WorkspaceHttpClient = {
      ...
      async request(path) {
        ...
        if (path === "/api/operations/op-unit") {
          return new Response(JSON.stringify({
            id: "op-unit", state: "completed", result: { worktreeId: "wt-2", unitId: "unit-pending" }, error: null,
          }), { status: 200, headers: { "content-type": "application/json", "retry-after": "0.001" } });
        }
