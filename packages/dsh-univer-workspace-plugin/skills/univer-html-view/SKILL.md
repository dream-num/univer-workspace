...
Use HTML binding attributes for text and standard form controls. Use JavaScript when the page needs
custom data processing, presentation or interaction. Both approaches can share the same cells.
...
Open the published Workspace URL to use this interface; opening the HTML file alone or in DSH's
raw HTML preview does not provide it.
...
`subscribeRange`, read [charts](references/charts.md).
