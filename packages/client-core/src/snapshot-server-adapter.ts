...
import { isWorkspaceRecord, type WorkspaceHttp } from "./http.js";
import { readWorkspaceReferenceScope } from "./reference-load-context.js";
import { workspaceSnapshotPrefix, type WorkspaceRuntimeScope } from "./runtime-target.js";
...
export class WorkspaceSnapshotServerAdapter implements ISnapshotServerService {
  ...
  public async getResourcesRequest(
    ...
    if (
      !isWorkspaceRecord(body["resources"]) ||
      Object.entries(body["resources"]).some(
        ([id, resource]) =>
          !params.resourceIDs.includes(id) ||
          !isWorkspaceRecord(resource) ||
          (resource["id"] !== undefined && resource["id"] !== id) ||
  ...
  public async saveSnapshot(context: ILogContext, params: ISaveSnapshotRequest): Promise<ISaveSnapshotResponse> {
    throw this.readOnly(context, params.unitID);
  }

  public async updateSnapshot(context: ILogContext, params: ISaveSnapshotRequest): Promise<ISaveSnapshotResponse> {
    throw this.readOnly(context, params.unitID);
  }

  public async saveSheetBlock(context: ILogContext, params: ISaveSheetBlockRequest): Promise<ISaveSheetBlockResponse> {
    throw this.readOnly(context, params.unitID);
  }

  public async saveChangeset(context: ILogContext, _params: ISaveChangesetRequest): Promise<ISaveChangesetResponse> {
    throw this.readOnly(context);
  }

  public async copyFileMeta(context: ILogContext, _params: ICopyFileMetaRequest): Promise<ICopyFileMetaResponse> {
    throw this.readOnly(context);
  }

  public async getLatestCsReqIdBySid(context: ILogContext, params: IGetLatestCsReqIdBySidRequest): Promise<IGetLatestCsReqIdBySidResponse> {
    throw this.readOnly(context, params.unitID);
  }
  ...
  private async getBlock(
    ...
    if (!isWorkspaceRecord(value) || value["id"] !== params.blockID) {
  ...
  private prefix(context: ILogContext, unitId: string): string {
    return workspaceSnapshotPrefix(readWorkspaceReferenceScope(context, unitId) ?? this.options.hostScope);
  }

  private readOnly(context: ILogContext, unitId?: string): Error {
    readWorkspaceReferenceScope(context, unitId);
    return workspaceError(
      "workspace-reference-source-read-only",
      "Workspace referenced Source Units are read-only.",
    );
  }
}
...
function decodeSnapshot(value: unknown, unitId: string, type: number): ISnapshot {
  ...
  if (type === 2 || type === 5) {
    ...
    if (!isWorkspaceRecord(workbook) || !isWorkspaceRecord(workbook["sheets"])) {
    ...
    if (!isWorkspaceRecord(metadata)) throw invalidResponse(`Workspace ${field} Snapshot is invalid.`);
    decoded[field] = {
      ...metadata,
...
function requireType(value: unknown): 1 | 2 | 3 | 5 | 6 {
  ...
  throw workspaceError(
    ...
    "Workspace references support Sheet, Doc, Slide, Base and Board Units.",
  );
}
...
function decodeBytes(value: unknown, subject: string): Uint8Array {
  ...
  if (typeof value !== "string" || !isBase64(value)) {
    throw invalidResponse(`Workspace ${subject} is not base64.`);
  }
