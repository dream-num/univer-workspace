...
export function parseWorkspaceRuntimeTarget(value: unknown): WorkspaceRuntimeTarget {
...
export function serializeWorkspaceRuntimeTarget(
  ...
  return {
    ...
    scope:
      target.scope.kind === "trunk"
        ? { kind: "trunk" }
        : { kind: "worktree", worktreeId: target.scope.worktreeId },
    unitId: target.unitId,
    unitType: target.unitType,
  };
}

function parseScope(value: unknown): WorkspaceRuntimeScope {
...
function normalizeOrigin(value: string): string {
  ...
  try {
    ...
    throw workspaceError("WORKSPACE_ORIGIN_INVALID", "Workspace origin is invalid");
  }
  if (
...
function nonEmpty(value: string, name: string): string {
  if (value.length === 0) {
    throw workspaceError("WORKSPACE_TARGET_INVALID", `${name} must not be empty`);
  }
  return value;
}
...
function invalidTarget(): Error {
  return workspaceError("WORKSPACE_TARGET_INVALID", "Workspace runtime target is invalid");
}
