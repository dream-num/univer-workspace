...
export interface WorkspaceSnapshotLoader {
  loadSheet(unitId: string, revision: number, context?: ILogContext, options?: SnapshotLoadOptions): Promise<LoadedUnit>;
  loadDoc(unitId: string, revision: number, context?: ILogContext, options?: SnapshotLoadOptions): Promise<LoadedUnit>;
  loadSlide(unitId: string, revision?: number, context?: ILogContext, options?: SnapshotLoadOptions): Promise<LoadedUnit>;
  loadBase(unitId: string, revision: number, context?: ILogContext, options?: SnapshotLoadOptions): Promise<LoadedUnit>;
  loadBoard(unitId: string, revision?: number, context?: ILogContext, options?: SnapshotLoadOptions): Promise<LoadedUnit>;
}
...
async function ensureReferencedUnit(
  ...
  if (ensureInput.ref.file.kind !== "self") {
    throw providerError("unsupported-file-kind", "Workspace references require a self ResourceRef.");
  }
