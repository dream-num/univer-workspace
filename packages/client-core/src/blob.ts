...
export class WorkspaceBlobFeature {
  ...
  /** Replace published bytes, preserving Resource identity. The ETag must come from the downloaded content. */
  public async replace(input: {
    readonly resourceId: string;
    readonly filePath: string;
    readonly etag: string;
    readonly idempotencyKey?: string;
  }): Promise<{ operationId: string; resourceId: string; etag: string }> {
    const resourceId = requireIdentity(input.resourceId, "Resource ID");
    if (!/^"[^"\r\n]+"$/.test(input.etag) || input.etag.length > 200) {
      throw workspaceError(
        "workspace-request-invalid",
        "etag must be the quoted strong ETag from the downloaded Blob.",
      );
    }
    const source = await inspectSource(input.filePath);
    ...
    const parseResult = (value: unknown) => {
      if (
        ...
        throw workspaceError("workspace-invalid-response", "Blob replacement result is invalid.");
      }
      return { operationId, resourceId, etag: value["etag"] };
    };
    try {
      return parseResult(
        await http.json(`/api/blob-resources/${encodeURIComponent(resourceId)}/content`, {
          method: "PUT",
      ...
      // A lost response may follow a committed replacement. Resolve the same Operation, never invent a new key.
      try {
        const operation = await http.json(`/api/operations/${encodeURIComponent(operationId)}`);
        const value = operation;
        if (
          isWorkspaceRecord(value) &&
          value["id"] === operationId &&
          value["kind"] === "replaceBlobContent" &&
          value["state"] === "completed"
        ) {
          return parseResult(value["result"]);
        }
      } catch {
        /* Preserve the stable recovery identity if the status request also fails. */
      }
      throw workspaceError(
        "workspace-result-unknown",
        "Blob replacement result is unknown. Inspect its Operation before retrying with the same key.",
