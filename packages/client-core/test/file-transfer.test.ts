...
describe("Workspace application feature parity", () => {
  it("replaces a Blob with the downloaded validator and resolves a lost response through its Operation", async () => {
    const directory = await temporaryDirectory();
    const filePath = join(directory, "edited.txt");
    await writeFile(filePath, "edited");
    const result = {
      operationId: "replace-key-00000001",
      resourceId: "blob-1",
      etag: '"new-validator"',
    };
    let puts = 0;
    const feature = new WorkspaceBlobFeature(
      authFor(async (input, init) => {
        ...
        if (request.method === "PUT") {
          puts++;
          expect(request.headers.get("if-match")).toBe('"downloaded-validator"');
          expect(request.headers.get("idempotency-key")).toBe(result.operationId);
          expect(request.headers.get("content-length")).toBe("6");
          expect(await request.text()).toBe("edited");
          throw new Error("response lost after commit");
        }
        expect(new URL(request.url).pathname).toBe(`/api/operations/${result.operationId}`);
        return jsonResponse({
  ...
  it("keeps stale and unknown replacements explicit without generating a second write", async () => {
    const directory = await temporaryDirectory();
    const filePath = join(directory, "edited.txt");
    await writeFile(filePath, "edited");
    const input = {
      ...
      filePath,
      etag: '"old"',
      idempotencyKey: "replace-key-00000002",
    };
    const stale = new WorkspaceBlobFeature(
      authFor(
        async () =>
          new Response(
            JSON.stringify({
              error: { code: "PRECONDITION_FAILED", message: "Download current content" },
            }),
            { status: 412 },
          ),
      ),
    );
    await expect(stale.replace(input)).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
    const unknown = new WorkspaceBlobFeature(
      authFor(async () => {
        throw new Error("offline");
      }),
    );
    await expect(unknown.replace(input)).rejects.toMatchObject({
      code: "workspace-result-unknown",
      detail: { operationId: input.idempotencyKey, resourceId: input.resourceId },
    });
    await expect(unknown.replace({ ...input, etag: "*" })).rejects.toMatchObject({
