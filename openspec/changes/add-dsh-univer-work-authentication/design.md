...
Change 1 can emit plain ESM because its Host entry has no private workspace imports. This Change invalidates that assumption: a tarball installed in a DSH profile cannot resolve private `@univerjs/univer-workspace-client-core@workspace:*`. The package therefore reuses the repository's installed Vite/Rollup toolchain to bundle the Host entry and inline reachable Client Core code. Node built-ins and the exact published DSH/Cordis runtime packages remain external. The Host Service Definition packages are exact optional peers, with exact dev dependencies for local build and test, so an installed plugin resolves them through DSH's profile module fallback and shares the Host's module identity instead of installing a second runtime graph.
