...
- Remove Typst's process-global deterministic-random mutation with a per-invocation standard-library context and keep CLI output deterministic.
...
workspace_typst_compile / workspace_typst_apply
         │
         ├── exact Core compiler ──> private artifact directory ─rename─> public artifacts
         │
         └── apply only: private deterministic VM context
...
### 2. Reuse the Change 5 local gate and publish by same-parent rename

Pure exact-key/type/cross-field/argument-budget validation runs before async policy. The pre-execute branch then positively checks public `LocalFileSystem` identity before resolving model paths, resolves the calling Session cwd, enforces containment and disjoint bundle/output roots, checks current read/write effect policy, verifies the destination is absent and asks once with fixed text. It retains no path or policy object across approval.

The accepted body repeats current provider, cwd, policy, containment and absence checks. It creates a mode-restricted private directory beside the destination, passes its `previews` child to Core, writes program/diagnostics there, validates actual byte totals and preview metadata, syncs completed files and then renames the directory to the still-absent destination. Same-parent rename keeps publication on one filesystem. Failures close files and remove the private directory in a non-cancellable finalizer; no path outside Session cwd, temporary basename or Host absolute path is returned.
...
### 5. Use a per-invocation Node VM context for deterministic random values
...
The materializer replaces `new Function` plus process-global random descriptor patching with Node's built-in VM compilation/context. Each invocation exposes the guarded `univerAPI`, an invocation-local `Math.random` and `crypto.getRandomValues` backed by the existing JavaScript-source stable seed. Concurrent calls therefore receive independent deterministic sequences and leave Host globals untouched. The existing exactly-one-Doc lifecycle guard, target identity, `save()`, `rev: 1`, name selection and `finally` disposal remain unchanged.
...
- **Node VM could be mistaken for a hostile-code sandbox** -> Accept only exact compiler output, keep the existing Facade guard and document that VM only isolates deterministic globals.
...
- **Output directory publication races another writer** -> Default to no-clobber, recheck absence and let final rename fail closed; add no overwrite mode.
...
2. Extend Core Typst inputs/materializer with optional signal, budgets and license; move deterministic execution to a per-invocation VM context while retaining existing CLI behavior tests.
3. Register the two tools and approval branches in the existing Host effect; add exact schemas, local artifact publication, safe error projection, cancellation and partial-side-effect finalizers.
