...
- [x] 1.1 确认 Changes 1–6 已实施并核对现有 DSH Host lifecycle、authenticated current runtime generation、local file/policy gate、Core render/load/write exports、DSH rc.2 ToolRuntime cancellation/pre-execute 顺序及SDK beta.2 screenshot/layout/browser schemas与限制；在现有owner模块导出`projectWorkspaceFileTransferDependencyFailure(error)`与`projectWorkspaceContentDependencyFailure(error)`，让既有file-transfer/content tools委托它们并以focused preservation tests证明完整allowlists、exact constructor checks与safe details不变，Render复用两者且不导出allowlist set、不新增generic/parallel adapter；运行两个package typecheck，任一其他prerequisite不符时停止，不创建第二runtime、file或render owner，也不改变冻结baseline。

## 2. 贯穿 Core render Unit 与 browser cancellation

- [x] 2.1 为 render Unit input、source opening、target/reference resolution、UnitData export、Worktree Asset resolution 和 layout load 追加 optional signal，并把既有 screenshot/lint signal 贯穿 browser construction/operation；增加 focused Core tests，覆盖 pre-abort、reference/Asset mid-abort、无后续步骤、browser success/failure/abort 全部先 close、不可中断 close 被等待，以及无 signal 的 target/reference/order/result compatibility，再运行相关 Client Core render/source/screenshot/lint tests。
