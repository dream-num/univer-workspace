...
import { createWorkspaceSvgTextMeasurer } from "../src/features/svg/text-measurer.js";
...
describe("Workspace SVG text measurer", () => {
  it("maps styled SVG runs into one unbounded Univer document line", async () => {
    ...
    expect(measureText.mock.calls[0]?.[0].doc).toMatchObject({
      body: {
        dataStream: "Hello 世界\r\n",
        paragraphs: [{ paragraphId: "svg-facade-measure-p0", startIndex: 8 }],
        textRuns: [
          { ed: 5, st: 0, ts: { bl: 1, ff: "Inter", fs: 15 } },
          { ed: 8, st: 5, ts: { fs: 12, it: 1 } },
        ],
      },
      documentStyle: {
        marginBottom: 0,
