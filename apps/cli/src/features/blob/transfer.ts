...
export class WorkspaceBlobFeature {
  ...
  public async get(
    ...
    return await getBlob(await this.auth.authenticatedHttp("client"), id);
  }
  ...
  public async upload(input: {
    ...
    for (let attempt = 0; attempt < this.maxAttempts; attempt += 1) {
      if (envelope.upload.state === "waitingForUpload") {
        envelope = await this.uploadBytes(http, envelope, intent, source);
        continue;
      }
      ...
      if (envelope.upload.state === "completed") {
        return await this.completedResult(http, envelope, intent);
      }
    ...
    try {
      ...
      try {
        ...
        if (refreshed.upload.state === "completed") {
          return await this.completedResult(http, refreshed, intent);
        }
        return undefined;
      } catch (statusError) {
        if (!isWorkspaceResultUnknown(statusError)) throw statusError;
  ...
  private async completedResult(
    ...
    const view = await getBlob(http, envelope.upload.resourceId);
