...
import { parseWorktree, type WorkspaceWorktree, type WorkspaceWorktreeState } from "./model.js";
import type { WorkspaceHttp } from "../../transport/http.js";
...
export class WorkspaceWorktreeFeature {
  public constructor(private readonly auth: WorkspaceAuth) {}
  ...
  public async list(input: ListWorktreesInput): Promise<readonly WorkspaceWorktree[]> {
    ...
    const body = await (
      await this.auth.authenticatedHttp("client")
    ).json(`/api/worktrees?${query.toString()}`);
    if (!Array.isArray(body["items"]))
      throw invalidResponse("Workspace response is missing Worktrees.");
  ...
  public async create(input: {
    ...
    const http = await this.auth.authenticatedHttp("client");
  ...
  public async update(
    ...
    const body = await (
      await this.auth.authenticatedHttp("client")
    ).json(`/api/worktrees/${encodeURIComponent(id)}`, {
      body: input,
      method: "PATCH",
    });
  ...
  public async transition(
    ...
    try {
      ...
      const confirmed = await getWorktree(http, id);
      if (confirmed.state === expected[action]) return confirmed;
      throw workspaceError(
...
function requireId(value: string, label: string): string {
  const id = value.trim();
  if (id === "") throw workspaceError("workspace-argument-invalid", `${label} must not be empty.`);
  return id;
}
