...
export class WorkspaceSnapshotServerAdapter implements ISnapshotServerService {
  ...
  public async getResourcesRequest(
    ...
    if (!isWorkspaceRecord(body["resources"])) {
  ...
  private prefix(context: ILogContext, unitId: string): string {
    const source = readWorkspaceReferenceScope(context, unitId);
    const scope = source ?? this.options.hostScope;
    return workspaceSnapshotPrefix(scope);
  }
...
function decodeSnapshot(value: unknown, unitId: string, type: number): ISnapshot {
  ...
  if (type === 2 || type === 5) {
    ...
    if (!isWorkspaceRecord(metadata)) {
      throw invalidResponse(`Workspace ${field} Snapshot is invalid.`);
    }
    decoded[field] = {
      ...metadata,
...
function parseError(value: unknown): IGetUnitOnRevResponse["error"] {
  ...
  if (!isWorkspaceRecord(value) || typeof value["code"] !== "number") {
