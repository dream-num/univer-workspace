...
Workspace Agent 是单人应用，每个实例只有一个当前账号。切换账号影响所有已打开的标签页；
页面通过 DSH 连接状态变化暂停旧界面并检查就绪状态。账号变化后，每个标签页都提示用户确认“刷新页面”；
同一账号的普通重连恢复后直接继续操作。限时检查失败后保留暂停状态，并提供“重新检查”。
账号服务切换期间，请求暂时返回 503；浏览器认证和会话附件归属由 DSH 负责。
...
OAuth 完成和退出登录使用最长 45 秒的短时就绪检查。DSH 连接状态变化或页面恢复时，也会检查就绪状态。
...
- **目录仍显示旧账号：** 授权后在受影响的标签页确认“刷新页面”。若持续存在，记录服务地址和账号名称用于诊断，保留本地数据目录与会话历史。
