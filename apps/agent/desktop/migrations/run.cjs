...
async function migrateRuntimeHome(resources, home, { targetVersion = CURRENT_SCHEMA_VERSION, registry = migrations, report = () => {} } = {}) {
  ...
  try {
    ...
    try { await rename(staging, home); unchanged = false; }
    catch (error) {
      if (backedUp) {
    ...
    // Only a completed failure with the original directory still active can
    // authorize installer binary rollback. Interruption produces no receipt.
    if (unchanged) error.runtimeHomeUnchanged = true;
    throw error;
  }
}
