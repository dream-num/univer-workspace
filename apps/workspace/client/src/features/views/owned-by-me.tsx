...
import { ownedByMeQueryOptions } from "./owned.queries";
...
export function OwnedByMe({
  ...
  const query = useQuery(ownedByMeQueryOptions);
      ...
      {query.data.items.length === 0 ? (
        ...
        <>
          ...
          >
            ...
            <span className="max-[720px]:hidden">{t("lastModified")}</span>
            <span aria-hidden="true" />
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2 max-[720px]:px-1.5">
            {visibleItems.length === 0 ? (
              <Empty className="mt-16" title={t("noMatchingResources")} />
            ) : (
