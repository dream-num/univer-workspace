...
} from "../../web/src/features/worktrees/preview-comparison-focus.js";
...
describe("preview comparison stable-ID focus", () => {
  ...
  it("selects the matching Doc paragraph and SectionBreak offsets", async () => {
    ...
    const api = {
      getActiveDocument: () => ({
        getParagraphs: () => [
          {
            getId: () => "p1",
            getInfo: () => ({ startOffset: 4, endOffset: 9 }),
          },
        ],
        getBody: () => ({
          sectionBreaks: [{ sectionId: "s1", startIndex: 12 }],
        }),
        setSelection,
      }),
    };
    ...
    await expect(
      focusPreviewComparisonTarget(
        ...
        "unused",
        { category: "paragraph", stableId: "p1" }
      )
    ).resolves.toBe(true);
    await expect(
      focusPreviewComparisonTarget(
  ...
  it("activates the matching Slide before checking its element", async () => {
    ...
    await expect(
      focusPreviewComparisonTarget(
        ...
        "unused",
        { category: "slide-element:page1", stableId: "shape1" },
        { selectSlideElement }
      )
    ).resolves.toBe(true);
  ...
  it("activates Base table and routes view, record, and field targets", async () => {
    ...
    await focusPreviewComparisonTarget(
      api,
      UniverInstanceType.UNIVER_BASE,
      "unused",
      { category: "view:t1", stableId: "v1" }
    );
    await focusPreviewComparisonTarget(
      api,
      UniverInstanceType.UNIVER_BASE,
      "unused",
      { category: "record:t1", stableId: "r1" }
    );
    await focusPreviewComparisonTarget(
      api,
      UniverInstanceType.UNIVER_BASE,
      "unused",
      { category: "field:t1", stableId: "f1" }
    );
  ...
  it("selects and centers a Board element in its own pane", async () => {
    ...
    vi.stubGlobal("document", {
      getElementById: (id: string) =>
        id === "board-pane" ? { clientWidth: 640, clientHeight: 360 } : null,
    });
    ...
    await expect(
      focusPreviewComparisonTarget(
        ...
        "board-pane",
        { category: "board-element:page1", stableId: "e1" }
      )
    ).resolves.toBe(true);
