...
export function createBlobsRouter(options: {
  ...
  router.put("/blob-resources/:resourceId/content", async (request, response) => {
    const result = await options.blobs.replace(session(request.headers.cookie), required(request.params.resourceId),
      request.headers["idempotency-key"], request.headers["if-match"], request.headers["content-length"], request);
