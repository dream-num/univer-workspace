...
export function createNativeComparisonHighlightController(input: {
  ...
  const refresh = async (): Promise<void> => {
    ...
    if (input.unitType === UniverInstanceType.UNIVER_SLIDE && observedScene !== render.scene) {
      ...
      if (selectedTarget !== undefined) {
        const viewportKey =
          input.unitType === UniverInstanceType.UNIVER_SHEET
            ...
            : input.unitType === UniverInstanceType.UNIVER_DOC
...
function objectKeyForItem(
  ...
  const slideId = item.category.startsWith("slide-element:")
    ...
    : item.scope?.entityType === "slide"
      ? item.scope.stableId
      : undefined;
  if (unitType === UniverInstanceType.UNIVER_SLIDE && slideId !== undefined) {
...
async function buildBaseTargets(
  ...
  return items.flatMap((item) => {
    ...
    const baseTableId = tableId ?? item.parentStableId;
    const [cellRecordId, ...cellFieldParts] = item.stableId.split(":");
    const cellFieldId = cellFieldParts.join(":");
    ...
    const matching = regions.filter(({ result }) => {
      const value = result;
      if (baseTableId !== undefined && value.tableId !== baseTableId) return false;
      ...
      if (category === "cell") {
        return (
          value.recordId === cellRecordId && value.fieldId === cellFieldId
        );
      }
      return false;
    });
