import type { IDocumentBody, IDocumentData, IParagraph, ITableRow, ITextStyle } from "@univerjs/core";
import {
  BooleanNumber,
...
import type { IUnitComparisonLeafChange } from "@univerjs-pro/edit-history";
import type {
  ComparisonSide,
  ComparisonTone,
...
export function decorateDocumentComparisonSide(
  ...
  for (const [segmentId, header] of Object.entries(decorated.headers ?? {})) {
    decorateBody(header.body, peer.headers?.[segmentId]?.body, side, comparison, tables, ["headers", segmentId, "body"]);
  }
  for (const [segmentId, footer] of Object.entries(decorated.footers ?? {})) {
    decorateBody(footer.body, peer.footers?.[segmentId]?.body, side, comparison, tables, ["footers", segmentId, "body"]);
  }
  return decorated;
}
      ...
      const item = comparison.items.find(
        (candidate) =>
          ...
          (candidate.entityType === "paragraph" ||
            candidate.entityType === "text-style")
      );
      const kind =
        row.presence === "left"
          ? "delete"
          : row.presence === "right"
            ...
            : item?.kind ?? "equal";
      return {
        id: `${segmentPath?.join("/") ?? "body"}:${row.stableId}:${index}`,
        stableId: row.stableId,
        kind,
        moved: item?.moved ?? false,
        left: projectParagraph(
          left,
          row.stableId,
          leftById.get(row.leftNativeStableId ?? "")
        ),
        right: projectParagraph(
          right,
          row.stableId,
          rightById.get(row.rightNativeStableId ?? "")
        ),
      };
    });
...
function projectParagraph(
  ...
  const structure = [...(body.tables ?? []), ...(body.columnGroups ?? []), ...(body.blockRanges ?? [])]
    .find((range) => span.end > range.startIndex && span.end < range.endIndex);
  return {
    stableId, paragraphId: span.id, index: span.index,
    start: span.start, end: span.end, text: body.dataStream.slice(span.start, span.end), value: span.paragraph,
...
function structuralRanges(body: IDocumentBody): StructuralRange[] {
  return [
    ...(body.tables ?? []).map((range) => ({
      entityType: "table-range" as const, stableId: range.tableId, ...getTableRangeInterval(range)
    })),
    ...(body.columnGroups ?? []).map((range) => ({
      entityType: "column-group" as const, stableId: range.columnGroupId, ...getColumnGroupRangeInterval(range)
    })),
    ...(body.blockRanges ?? []).map((range) => ({
      entityType: "block-range" as const, stableId: range.blockId, ...getBlockRangeInterval(range)
    })),
  ];
}
...
function insertMissingContent(
  ...
  const missingRanges = sourceRanges.filter((range) => items.some((item) =>
    item.entityType === range.entityType && item.kind === missingKind &&
  ...
  const rootFragments = missingRanges.filter((range) => !sourceRanges.some((parent) =>
    parent !== range && parent.startOffset <= range.startOffset && parent.endOffset >= range.endOffset
  )).map((range) => {
    const rowIndices = rows.flatMap((row, index) => {
      const source = row[peerSide];
      return source !== null && source.end >= range.startOffset && source.end < range.endOffset ? [index] : [];
    });
  ...
  for (let rowIndex = rows.length - 1; rowIndex >= 0; rowIndex -= 1) {
    ...
    const nestedOffset = fragment === undefined && source?.structureId !== undefined
      ? nestedParagraphInsertionOffset(current, sourceContainers, rows, rowIndex, side, ghostParagraphIds)
      : undefined;
    if (fragment === undefined && source?.structureId !== undefined && nestedOffset === undefined) continue;
    const insertionBody = fragment === undefined
      ? paragraphInsertionBody(peer, source!)
      : Tools.deepClone(getBodySliceForTextXAction(peer, fragment.range.startOffset, fragment.range.endOffset, false));
    const nextRow = (fragment?.lastRow ?? rowIndex) + 1;
    const insertionOffset = nestedOffset ?? ghostInsertionOffset(current, rows.slice(nextRow), side, ghostParagraphIds);
    ...
    for (const candidate of fragment === undefined ? [row] : rows.slice(rowIndex, nextRow)) {
      ...
      if (candidate[side] === null && ghostId !== undefined) ghostParagraphIds.set(candidate.id, ghostId);
    }
  }
...
function paragraphContainers(body: IDocumentBody): ParagraphContainer[] {
  ...
  const containers: ParagraphContainer[] = (body.blockRanges ?? []).map((block, index) => ({
    kind: "block", parentId: block.blockId, nativeId: block.blockId, index,
    ...getBlockRangeInterval(block), contentEnd: block.endIndex,
  }));
  const sections = new Map(body.sectionBreaks?.map((section) => [section.startIndex, section.sectionId]));
  for (const table of body.tables ?? []) {
    ...
    for (let offset = table.startIndex + 1; offset < table.endIndex; offset++) {
      ...
      containers.push({ kind: "row", parentId: table.tableId, nativeId: undefined, index: rowIndex, ...row, contentEnd: row.endOffset - 1 });
      let cellIndex = 0;
      for (let cellOffset = offset + 1; cellOffset < row.endOffset; cellOffset++) {
        ...
        const contentEnd = body.dataStream[interval.endOffset - 2] === T.SECTION_BREAK
          ? interval.endOffset - 2 : interval.endOffset - 1;
        containers.push({ kind: "cell", parentId: table.tableId, nativeId: sections.get(contentEnd), index: cellIndex++, rowIndex, ...interval, contentEnd });
  ...
  for (const group of body.columnGroups ?? []) {
    ...
    for (let offset = group.startIndex + 1; offset < group.endIndex; offset++) {
      ...
      containers.push({
        kind: "column", parentId: group.columnGroupId, nativeId: group.columns?.[columnIndex]?.columnId,
        index: columnIndex, startOffset: offset, endOffset, contentEnd: endOffset - 1,
      });
      columnIndex++;
      offset = endOffset - 1;
    }
  }
  return containers.sort((left, right) => (left.endOffset - left.startOffset) - (right.endOffset - right.startOffset));
}
...
function columnEndOffset(stream: string, start: number): number | undefined {
  ...
  for (let offset = start; offset < stream.length; offset++) {
    ...
    else if (stream[offset] === DataStreamTreeTokenType.COLUMN_END && --depth === 0) return offset + 1;
  }
  return undefined;
}
...
function matchingContainer(
  ...
  const candidates = targets.filter((target) => target.kind === source.kind && target.parentId === source.parentId);
  if (source.nativeId !== undefined) {
    const native = candidates.find((target) => target.nativeId === source.nativeId);
    if (native !== undefined) return native;
  }
  if (source.kind === "row") {
    const cellIds = new Set(sources.filter((cell) => cell.kind === "cell" && cell.parentId === source.parentId && cell.rowIndex === source.index)
      .flatMap((cell) => cell.nativeId === undefined ? [] : [cell.nativeId]));
    const cell = targets.find((target) => target.kind === "cell" && target.parentId === source.parentId && target.nativeId !== undefined && cellIds.has(target.nativeId));
    if (cell !== undefined) return candidates.find((target) => containsParagraph(target, cell.startOffset + 1));
  }
  const peerSide = side === "left" ? "right" : "left";
  for (const row of rows) {
  ...
  if (source.kind === "cell") {
    const parent = sources.find((candidate) => candidate.kind === "row" && candidate.parentId === source.parentId && candidate.index === source.rowIndex);
    const targetRow = parent === undefined ? undefined : matchingContainer(parent, sources, targets, rows, side, spans);
    if (targetRow !== undefined) {
      const sourceCells = sources.filter((candidate) => candidate.kind === "cell" && candidate.parentId === source.parentId && candidate.rowIndex === source.rowIndex);
      ...
      if (sourceCells.length === targetCells.length) return targetCells.find((candidate) => candidate.index === source.index);
    }
  }
  return undefined;
}
...
function insertMissingSlots(
  ...
  const candidates = sources.filter((source) => {
    ...
    const contained = rows.filter((row) => row[peerSide] !== null && containsParagraph(source, row[peerSide]!.end));
    return contained.length > 0 && contained.every((row) => row[side] === null) &&
      matchingContainer(source, sources, initialTargets, rows, side, initialSpans) === undefined;
  });
  const fragments = candidates.filter((source) => !candidates.some((parent) => parent !== source &&
    parent.startOffset < source.startOffset && parent.endOffset > source.endOffset
  ...
  for (const source of fragments) {
    ...
    const parentRow = source.kind === "cell" ? sources.find((candidate) => candidate.kind === "row" &&
      candidate.parentId === source.parentId && candidate.index === source.rowIndex) : undefined;
    const targetRow = parentRow === undefined ? undefined : match(parentRow);
    const parent = source.kind === "cell" ? targetRow : structuralRanges(current).find((range) =>
      range.stableId === source.parentId && range.entityType === (source.kind === "column" ? "column-group" : "table-range")
    );
    ...
    const next = rows.slice(lastRow + 1).flatMap((row) => {
      ...
      const target = targets.find((candidate) => candidate.kind === source.kind && candidate.parentId === source.parentId &&
        candidate.startOffset > parent.startOffset && candidate.endOffset < parent.endOffset && containsParagraph(candidate, span.end));
      return target === undefined ? [] : [target];
    })[0];
    const index = next?.index ?? targets.filter((candidate) => candidate.kind === source.kind && candidate.parentId === source.parentId &&
      (source.kind !== "cell" || candidate.rowIndex === targetRow?.index)).length;
    if (!insertSlotMetadata(current, peer, source, index, targetRow?.index, tables)) continue;
    const insertion = Tools.deepClone(getBodySliceForTextXAction(peer, source.startOffset, source.endOffset, false));
    const ids = insertGhostBody(current, insertion, next?.startOffset ?? parent.endOffset - 1, side, source.startOffset);
    for (const row of rows) {
      const paragraph = row[peerSide];
      if (row[side] !== null || paragraph === null || !containsParagraph(source, paragraph.end)) continue;
...
function insertSlotMetadata(
  ...
  if (source.kind === "column") {
    const group = current.columnGroups?.find((candidate) => candidate.columnGroupId === source.parentId);
    const descriptor = peer.columnGroups?.find((candidate) => candidate.columnGroupId === source.parentId)?.columns?.[source.index];
    if (group === undefined) return false;
    if (descriptor !== undefined) (group.columns ??= []).splice(index, 0, Tools.deepClone(descriptor));
    return true;
  }
  ...
  const width = Math.max(...table.tableRows.map((row) => (row.gridBefore ?? 0) + (row.gridAfter ?? 0) +
  ...
  if (added > 0) {
    ...
    table.size = { ...table.size, width: { ...table.size.width, v: table.tableColumns.reduce((sum, column) => sum + (column.size.width?.v ?? 0), 0) } };
  }
  return true;
}
...
function tableCellGridColumn(row: ITableRow, index: number): number {
  ...
  return (row.gridBefore ?? 0) + row.tableCells.slice(0, index).reduce((sum, cell) => sum + (cell.columnSpan ?? 1), 0);
}
...
function decorateCellPresence(
  ...
  for (const row of rows) {
    const own = row[side];
    if (own !== null && (row.left === null || row.right === null)) tones.set(own.paragraphId, row.moved ? "update" : "insert");
  }
  const paragraphs = paragraphSpans(body);
  for (const container of paragraphContainers(body).filter((candidate) => candidate.kind === "cell")) {
    ...
    if (tone === undefined || !contained.every((paragraph) => tones.get(paragraph.id) === tone)) continue;
    const cell = tables?.[container.parentId]?.tableRows[container.rowIndex!]?.tableCells[container.index];
...
function nestedParagraphInsertionOffset(
  ...
  const target = matchingContainer(container, sourceContainers, paragraphContainers(current), rows, side, spans);
  ...
  for (const row of rows.slice(rowIndex + 1)) {
    ...
    if (span !== undefined && containsParagraph(target, span.end)) return paragraphInlineStart(current, span.start);
  }
...
function ghostInsertionOffset(
  ...
  for (const row of nextRows) {
    ...
    return structuralRanges(current).reduce((offset, range) =>
      range.startOffset <= span.end && span.end < range.endOffset ? Math.min(offset, range.startOffset) : offset,
    span.start);
  }
...
function insertGhostBody(
  ...
  const insertedIds = new Map(insertion.paragraphs?.map((paragraph) => [offset + paragraph.startIndex, paragraph.paragraphId]));
...
function paragraphInlineStart(body: IDocumentBody, offset: number): number {
  ...
  const inlineTokens: readonly string[] = [
    DataStreamTreeTokenType.CUSTOM_RANGE_START, DataStreamTreeTokenType.CUSTOM_RANGE_END, DataStreamTreeTokenType.CUSTOM_BLOCK
  ];
...
function findSentinelOffset(body: IDocumentBody): number {
  ...
  return sentinel?.startIndex ?? (body.dataStream.endsWith(DataStreamTreeTokenType.SECTION_BREAK)
    ? body.dataStream.length - 1 : body.dataStream.length);
}
...
function paragraphSpans(body: IDocumentBody | undefined): ParagraphSpan[] {
  ...
  return paragraphs.map((paragraph, index) => {
    ...
    return { id: paragraph.paragraphId!, index: indices.get(paragraph)!, start, end: paragraph.startIndex, paragraph };
  });
}

function buildTextRanges(
  segments: NonNullable<IUnitComparisonLeafChange["segments"]>,
  leftStart: number,
  rightStart: number
  ...
  while (li < segments.left.length || ri < segments.right.length) {
    ...
    if (before !== undefined && lo === before.text.length) { li += 1; lo = 0; continue; }
    if (after !== undefined && ro === after.text.length) { ri += 1; ro = 0; continue; }
  ...
  for (let index = 0; index < chunks.length;) {
    ...
    if (leftLength > 0) left.push({ start: leftOffset, end: leftOffset + leftLength, tone: hasInsertion ? "update" : "delete" });
    if (rightLength > 0) right.push({ start: rightOffset, end: rightOffset + rightLength, tone: hasDeletion ? "update" : "insert" });
...
function copyMissingRootObjects(
  ...
  const peerSide = side === "left" ? "right" : "left";
  const missingKind = side === "left" ? "insert" : "delete";
  for (const [key, entityType] of [["tableSource", "table"], ["drawings", "drawing"]] as const) {
    const targetRecord = asRecord(target[key]) ?? {};
    const sourceRecord = asRecord(source[key]) ?? {};
    const missingIds = new Set(items.filter((item) => item.entityType === entityType && item.kind === missingKind)
...
function decorateTables(
  ...
  for (const item of items.filter((candidate) => candidate.entityType === "table")) {
    ...
    for (const change of item.changes) {
      ...
      if (rowIndex >= 0 && cellIndex >= 0) changedCells.add(`${path[rowIndex + 1]}:${path[cellIndex + 1]}`);
