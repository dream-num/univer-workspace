...
export async function focusPreviewComparisonTarget(
  ...
  if (unitType === UniverInstanceType.UNIVER_DOC) {
    ...
    if (target.category === "section") {
      const section = document
        .getBody()
        .sectionBreaks?.find(
          (candidate) => candidate.sectionId === target.stableId
        );
  ...
  if (unitType === UniverInstanceType.UNIVER_SLIDE) {
  ...
  if (
    ...
    target.category.startsWith("board-element")
  ) {
